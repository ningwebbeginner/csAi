...
(defun generate-at-most-h (n c current k)
  	(cond
	  ((> current k) nil)
	  (t (cons (list (* -1 (node2var n c k)) (* -1 (node2var n current k)) ) (generate-at-most-h n c (+ 1 current) k)))	  
	 );end cond
)
...
(defun at-most-one-color (n c k)
  (cond
	  ((> c k) nil)
	  ((= c k) nil)
	  (t (append (generate-at-most-h n c (+ 1 c) k) (at-most-one-color n (+ 1 c) k)))
	 )	
  )
...
(defun generate-edge-clauses (e k)
    (generate-edge-clauses-h (first e) (second e) 1 k)
  )

(defun generate-edge-clauses-h (f s c k)
  	(cond
	  	((> c k) nil)
		(t (cons (list (* -1 (node2var f c k)) (* -1 (node2var s c k))) (generate-edge-clauses-h f s (+ 1 c) k)))
	 ); end cond
 )
